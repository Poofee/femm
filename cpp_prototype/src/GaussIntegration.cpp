...
const std::array<std::array<double, 2>, 6> GaussIntegration::gauss1DData = {{
    {{0.0, 2.0}},                           // 1 point
...
const std::array<std::array<double, 4>, 7> GaussIntegration::gaussTriangularData = {{
...
const std::array<std::array<double, 5>, 5> GaussIntegration::gaussTetrahedralData = {{
...
std::vector<GaussIntegration::IntegrationPoint> GaussIntegration::get1DPoints(int nPoints) {
    ...
    for (size_t i = 0; i < nPoints; ++i) {
        points.emplace_back(data[i][0], 0.0, 0.0, data[i][1]);
    }
...
std::vector<GaussIntegration::IntegrationPoint> GaussIntegration::getQuadrilateralPoints(int nPoints) {
    ...
    for (size_t i = 0; i < nPoints; ++i) {
        for (size_t j = 0; j < nPoints; ++j) {
            double weight = xiData[i][1] * etaData[j][1];
            points.emplace_back(xiData[i][0], etaData[j][0], 0.0, weight);
        }
    }
    ...
    for (size_t i = 0; i < nPoints; ++i) {
        for (size_t j = 0; j < nPoints; ++j) {
            for (size_t k = 0; k < nPoints; ++k) {
                double weight = xiData[i][1] * etaData[j][1] * zetaData[k][1];
                points.emplace_back(xiData[i][0], etaData[j][0], zetaData[k][0], weight);
            }
        }
    }
...
std::vector<GaussIntegration::IntegrationPoint> GaussIntegration::getTriangularPoints(int order) {
    ...
    for (int i = 0; i < nPoints; ++i) {
        points.emplace_back(data[i][0], data[i][1], 0.0, data[i][3]);
    }
...
std::vector<GaussIntegration::IntegrationPoint> GaussIntegration::getTetrahedralPoints(int order) {
    ...
    for (int i = 0; i < nPoints; ++i) {
        points.emplace_back(data[i][0], data[i][1], data[i][2], data[i][4]);
    }
